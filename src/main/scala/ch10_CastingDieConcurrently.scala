...
object ch10_CastingDieConcurrently {
  ...
  def main(args: Array[String]): Unit = {
    ...
    unsafeRunTimedIO(List.fill(100)(IO.sleep(1.second).flatMap(_ => castTheDie())).parSequence.map(_.sum))
    ...
    { // IO.delay vs IO.sleep
      ...
      unsafeRunTimedIO(List.fill(3)(threadSleep).parSequence)(runtime)

      println("IO.sleep concurrently in 3 fibers on a single thread:")
      unsafeRunTimedIO(List.fill(3)(fiberSleep).parSequence)(runtime)
    }
  }
